...
> 状态：历史实现计划，不再作为当前精修器开发依据。当前可执行口径以 `docs/knowledge-content-standard.md`、`docs/quality-refine.md` 和 `scripts/quality_standard.mjs` 为准。本文中“8 维”“CLI 路径”等描述均是旧计划语境；现实现已升级为 9 维、API 模式、模型池/队列、图解形态适配、视觉 QA 与联网事实依据接口。
